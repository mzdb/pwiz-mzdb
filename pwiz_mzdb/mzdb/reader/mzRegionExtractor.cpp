...
const char *mzRegionExtractor::sqlMSnRTree = "SELECT bounding_box.* FROM bounding_box WHERE bounding_box.id IN (SELECT id FROM bounding_box_msn_rtree WHERE min_mz >= ? AND max_mz <= ? AND min_time >= ? AND max_time <= ?);";
...
PWIZ_API_DECL void mzRegionExtractor::rTreeExtraction(double minmz, double maxmz, double minrt, double maxrt, int msLevel, vector<mzScan*>& results) {
    ...
    if (msLevel > 1)
        sqlite3_prepare_v2(_mzdb.db, mzRegionExtractor::sqlMSnRTree, -1, &(_mzdb.stmt), 0);
    else
    ...
    for (auto it = BBs.begin(); it != BBs.end(); ++it) {
        ...
        int scanCount = IBlobReader::buildMapPositions(&(b.data[0]), b.size, scansInfos, dataEncodings);
        //vector<mzScan*> scans;
        ...
        for (int i = 1; i <= scanCount; ++i) {
            ...
            IBlobReader::readData(&(b.data[0]), b.size, i, s, scansInfos);
            results.push_back(s);
        }
        ...
        for (size_t j = 1; j < bbs.size(); ++j) {
            ...
            int scanCount_ = IBlobReader::buildMapPositions(&(bb.data[0]), bb.size, scansInfos_, dataEncodings);

            for (int i = 1; i <= scanCount_; ++i) {
                IBlobReader::readData(&(bb.data[0]), bb.size, i, results[i-1], scansInfos_);
            }
        }
    }
