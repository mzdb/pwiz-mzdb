 ...
 * Visual Studio 2012" installed on your system.
 *
 */
...
#include <stdint.h>
    ...
    /// filename will be copied. May be NULL only if 'sql_filename_buflen' is zero.
    ///
    ...
    /// sufficient to only specify the .d directory here).
    ///
    ...
    ///
    DllExport uint32_t baf2sql_get_sqlite_cache_filename_v2 (
        char *sql_filename_buf,
        uint32_t sql_filename_buflen,
        const char *baf_filename,
    ...
    /// calibration of the acquisition instrument. Using the Bruker DataAnalysis software,
    /// more sophisticated post-acquisition calibration can be performed; that improved
    /// calibration status will be stored in a file called 'Calibrator.ami' which is in
    /// the same directory as the original BAF. Baf2sql_C takes into account this
    /// information and returns properly calibrated data UNLESS you set the parameter
    /// ignore_calibrator_ami to the value 1 -- in that case the raw calibration is always
    /// returned, and the contents of Calibrator.ami are ignored.
    ///
    ...
    /// string describing the problem.
    ///
    /// \param ignore_calibrator_ami if set to 1, will always return raw m/z data from the
    /// BAF file; if set to 0 and a Calibrator.ami file is present in the same directory
    /// as the BAF, will use the calibration status stored there.
    ///
